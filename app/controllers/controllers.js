  ...
  EventCal.create(evecal, (err, data) => {
    if (err)
      res.status(500).send({
        status:false,
        message:
          err.message || "Some error occurred while creating the events calendar."
      });
      res.status(200).send({
        success:true,
  ...
  EventCal.getAll(title, (err, data) => {
    if (err)
      res.status(500).send({
        status:false,
        message:
          err.message || "Some error occurred while retrieving events calendar."
      });
